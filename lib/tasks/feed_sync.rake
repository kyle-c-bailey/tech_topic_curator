  ...
  def generate_three_word_phrases
    ...
    Entry.last_day.each do |entry|
      ...
      title_words.each_with_index do |word, index|
        ...
        example = "#{clean_word(word, false)} #{clean_word(title_words[next_word_index], false)} #{clean_word(title_words[two_word_index], false)}"
        ...
        if phrase_hash.has_key?(phrase_content)
          ...
          phrase_hash[phrase_content] = {example: example, sources: [entry.id]}
        end
      end
    end

    phrase_hash.delete_if { |phrase, meta| meta[:sources].size < 5 }
    phrase_hash.each do |phrase, meta|
      meta[:sources].each do |entry_id|
        Phrase.create_or_increment(phrase, entry_id, meta[:example])
      end
    end
  end
  ...
  def generate_two_word_phrases
    ...
    Entry.last_day.each do |entry|
      ...
      title_words.each_with_index do |word, index|
        ...
        example = "#{clean_word(word, false)} #{clean_word(title_words[next_word_index], false)}"
    ...
    Entry.last_day.each do |entry|
      ...
      title_words.each do |word|
        ...
        if phrase_hash.has_key?(word)
          ...
          phrase_hash[word] = {example: example, sources: [entry.id]}
        end
      end
    end
